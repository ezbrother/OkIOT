...
public class PitchDetector implements Runnable {
    ...
    private final static int MAX_FREQUENCY = 400; // HZ - it's for guitar,
    // should be enough
    private final static int DRAW_FREQUENCY_STEP = 5;

    public native void DoFFT(double[] data, int size);  // an NDK library 'fft-jni'

    public PitchDetector(SpeechSampleActivity parent, Handler handler) {
        parent_ = parent;
        handler_ = handler;
    ...
    public void run() {
        ...
        while (!Thread.interrupted()) {
            ...
            PostToUI(frequencies);
        }
    }

    private void PostToUI(HashMap<Double, Double> frequencies) {
        ...
        handler_.post(new Runnable() {
            public void run() {
                parent_.ShowPitchDetectionResult(temp);
            }
        });
    }
    ...
    private SpeechSampleActivity parent_;
